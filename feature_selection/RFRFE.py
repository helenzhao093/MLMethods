...
class RFRFE():
    def __init__(self, n_estimators=1000, step=0.1):
    ...
    def fit(self, X, y):
        ...
        self.selected_indices.append(np.array([i for i in range(len(X_train[0]))]))
    ...
    def transform(self, X, scoring):
        max_index = self.get_max(scoring)
        return X[:, self.selected_indices[max_index]]
        
    def get_max(self, scoring):
        return max(self.results.loc[rfrfe.results[scoring] == self.results.max()[scoring]].index)